...
{
	this->sm_msg_hook = SetWindowsHookEx(WH_GETMESSAGE, GetMsgHookProc, NULL, GetCurrentThreadId());
...
LRESULT CALLBACK MyWebBrowser::GetMsgHookProc(int n_code, WPARAM w_param, LPARAM l_param)
{
	...
	if (n_code >= 0 && PM_REMOVE == w_param)
	{
		...
		case WM_SYSKEYUP:
			...
			eat_keystroke = (p->vkCode == VK_TAB && (p->flags & LLKHF_ALTDOWN) ||
				p->vkCode == 46 && (p->flags & LLKHF_ALTDOWN) && (GetKeyState(VK_CONTROL) & 0x8000));
			break;
		}
...
// 		}
	...
	return (eat_keystroke ? 1 : CallNextHookEx(sm_msg_hook, n_code, w_param, l_param));
}