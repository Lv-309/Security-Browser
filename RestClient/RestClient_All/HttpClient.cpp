...
namespace ISXHttpClient 
{
	...
	{	
		...
		std::wstring str = L"/webservice/rest/server.php?&wstoken=859b2244c55636be03408c2b0e208b03&wsfunction=core_auth_request_password_reset&username=ws_access&email=taras.serhii@gmail.com";
			...
			{		
				...
				std::size_t lenght = (std::size_t)response.headers().content_length();		
				
				istream bodyStream = response.body();
		
				container_buffer<std::string> inStringBuffer;
				return bodyStream.read(inStringBuffer, lenght).then([inStringBuffer](std::size_t bytesRead)
				{
	...
	pplx::task<void> HttpClient::request_files_upload(char* buffer, std::size_t lenght)
	{		
		...
		{	
			...
			std::size_t lenght = (size_t)response.headers().content_length();
