		...
		: m_hinstance(h_hinstance)
{
	...
	this->Register(this->MsgRouter, this->m_lpclass_name, this->m_hinstance);
}

VOID MyWindow::Register(WNDPROC proc, LPCTSTR name, HINSTANCE hinst) const
{
	WNDCLASSEX wcex;
	...
	wcex.hIcon			= LoadIcon(this->m_hinstance, MAKEINTRESOURCE(IDI_WEBBROWSER2));
	wcex.hIconSm		= LoadIcon(this->m_hinstance, MAKEINTRESOURCE(IDI_SMALL));
	wcex.lpszMenuName	= NULL;

	if (this->Register(&wcex, proc, name, hinst) == FALSE)
		// TODO: Throw error
		throw "Cannot register window class";
}

ATOM MyWindow::Register(LPWNDCLASSEX wcex, WNDPROC proc, LPCTSTR name, HINSTANCE hinst) const
{
...
{
	return this->Show(SHOW_FULLSCREEN);
}
		...
		LPARAM l_param)
{
	...
	MyWindow* wb_window = (MyWindow*)GetWindowLong(hwnd, GWL_USERDATA);
