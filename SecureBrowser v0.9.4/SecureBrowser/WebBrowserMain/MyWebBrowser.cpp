...
MyWebBrowser::MyWebBrowser(HINSTANCE h_instance, LPCTSTR lpsz_link)
	: MyWindow(WB_WINDOW_NAME, h_instance, WB_WINDOW_CLASS_NAME)
	, m_handle_monitors()
...
ErrorTypes MyWebBrowser::GetErrorsState() const noexcept
{
...
ErrorTypes MyWebBrowser::GetMonitorsState() const noexcept
{
...
ErrorTypes MyWebBrowser::StartAuthentication(HWND hwnd, const RECT& rc_client) noexcept
{
...
ErrorTypes MyWebBrowser::StartTestPassing(HWND hwnd, const RECT& rc_client) noexcept
{
	// Create window of our web browser
	// in client side case
	if ((sm_lpwb_wnd = new WebBrowserWindow(hwnd)) == nullptr)
...
{	
	...
	case WM_ON_AUTHWND_CLOSED:
		...
		if ((this->m_lpface_detector = new FaceDetector(0)) == nullptr)
		{
		...
		try
		{
			this->m_async_cum_thread = std::async(std::launch::async, &FaceDetector::SaveWithInterval, this->m_lpface_detector, 10);
		}
