...
{
	if (m_handle_monitors.Verify() != ErrorTypes::ISOK)
...
MyWebBrowser* MyWebBrowser::CreateSafe(HINSTANCE h_instance, LPCTSTR lpsz_link)
{
	...
	VerifyLink		m_chk_link(lpsz_link);
	if (m_chk_link.Verify() != ErrorTypes::ISOK)
		return nullptr;
	// Verify session on remote or virtual access
	HandleSession	m_handle_session;
	if (m_handle_session.Verify() != ErrorTypes::ISOK)
		return nullptr;
