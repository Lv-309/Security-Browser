...
using namespace ISXAuthenticationWindow;

AuthenticationWindow::AuthenticationWindow(HINSTANCE hInst)
...
ErrorTypes AuthenticationWindow::GetErrorsState() const noexcept
{
	return ErrorTypes::IS_OK;
}

void AuthenticationWindow::CvMatToWinControl(const cv::Mat& img, HWND WinCtrl)
{
...
HWND AuthenticationWindow::CreateButton(HWND hParent, int x, int y, int width, int higth, LPCWSTR button_name, HMENU hMenu)
{
...
//HWND AuthenticationWindow::CreateListOfCameras(HWND hParent, int x, int y, int width, int higth, HMENU hMenu)
//{
...
//void AuthenticationWindow::AddCameraItems()
//{
...
HWND AuthenticationWindow::CreateAuthWindow(HWND hParent)
{
	INT w = 800, h = 700;
	...
	return CreateWindowEx(0, WINDOW_CLASS_NAME, WINDOW_NAME,
			NULL, x, y, w, h, hParent, NULL, m_hinstance, this);
}

void  AuthenticationWindow::SendToMoodle()
{
	...
	client.SetConfigFile("RestClient//config.txt");
	subj.setBufferData((char*)buffer.data(), buffer.size(), L"ID.jpg");

	//TO DO display msg "Your photo is being saved, wait a few seconds."
}

LRESULT CALLBACK AuthenticationWindow::WndProc(HWND hwnd, UINT u_msg, WPARAM w_param, LPARAM l_param)
{
	HWND parent_window;
	switch (u_msg)
	{
	case WM_CREATE:
		...
		m_hwnd_take_photo_button =
			CreateButton(hwnd, rect.left + 20, rect.bottom - 75, BUTTON_WIDTH, BUTTON_HIGTH,
				BUTTON_TAKE_PHOTO_NAME, (HMENU)WndCtrls::ID_TAKE_PHOTO_BTN);
		m_hwnd_retake_photo_button =
			CreateButton(hwnd, rect.left + 150, rect.bottom - 75, BUTTON_WIDTH, BUTTON_HIGTH,
				BUTTON_RETAKE_PHOTO_NAME, (HMENU)WndCtrls::ID_RETAKE_PHOTO_BTN);
		m_hwnd_submit_button =
			CreateButton(hwnd, rect.left + 280, rect.bottom - 75, BUTTON_WIDTH, BUTTON_HIGTH,
				BUTTON_SUBMIT_NAME, (HMENU)WndCtrls::ID_SUBMIT_BTN);

		//m_hwnd_list_of_cameras =
		//	CreateListOfCameras(hwnd, rect.right - 200, rect.bottom - 75, 175, 125, (HMENU)WndCtrls::ID_LIST_OF_CAMERAS_CBOX);

		m_hwnd_text_choose_camera = CreateWindowEx(0, TEXT("STATIC"), TEXT(/*"Choose camera:"*/ "NOTE\nSubmitting the photo can take a while."),
			WS_CHILD | WS_VISIBLE,
			//rect.right - 315, rect.bottom - 75, 115, 25,
			rect.right - 315, rect.bottom - 75, 155, 55,
	...
	case WM_PAINT:
	{
		...
		INT cam = 1; //m_camera_handler.GetNumOfAvailableCameras();
	...
	case WM_COMMAND:
		...
		{
		case (UINT)WndCtrls::ID_TAKE_PHOTO_BTN:
			...
			break;
		case (UINT)WndCtrls::ID_RETAKE_PHOTO_BTN:
			is_capture_current_frame = FALSE;
			...
			break;
		case (UINT)WndCtrls::ID_SUBMIT_BTN:
...
// 			parent_window = FindWindow(TEXT("WebBrowserWindow"), NULL);
// 			PostMessage(parent_window, WM_ON_AUTHWND_CLOSED, NULL, NULL);
