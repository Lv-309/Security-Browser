		...
		, hwnd_parent(_hwnd_parent)
{
	...
	// ShowWindow(this->GetControlWindow(), SW_SHOW);
	// this->Navigate(_T("softserve.academy"));
}

bool WebBrowserWindow::CreateBrowser()
{
	HRESULT hr = ::OleCreate(CLSID_WebBrowser, IID_IOleObject,
		OLERENDER_DRAW, 0, this, this, (void**)&this->ole_object);
	...
	hr = this->ole_object->SetClientSite(this);
	hr = OleSetContainedObject(this->ole_object, TRUE);

	RECT pos_rect;
	::SetRect(&pos_rect, -300, -300, 300, 300);
	hr = this->ole_object->DoVerb(OLEIVERB_INPLACEACTIVATE,
	...
	hr = this->ole_object->QueryInterface(&this->web_browser2);
...
{
	this->web_browser2->Refresh();
}

HRESULT WebBrowserWindow::GetURL(LPBSTR lpbstr_url) const
{
	return this->web_browser2->get_LocationURL(lpbstr_url);
}
...
VOID WebBrowserWindow::Navigate(WSTRING sz_url) const
{
	...
	this->web_browser2->Navigate(url, &flags, 0, 0, 0);
}
...
void WebBrowserWindow::SetRect(const RECT& rect)
{
	...
	this->ole_object->SetExtent(DVASPECT_CONTENT, &sz);

	if (this->ole_in_place_object != 0)
	{
		this->ole_in_place_object->SetObjectRects(&this->r_object,
			&this->r_object);
	}
}
...
{
	OleLockRunning(this->ole_object, TRUE, FALSE);
	this->ole_object->QueryInterface(&this->ole_in_place_object);
	this->ole_in_place_object->SetObjectRects(&this->r_object,
		&this->r_object);
...
{
	if (this->hwnd_control != 0)
		return this->hwnd_control;

	if (this->ole_in_place_object == 0)
		return 0;

	this->ole_in_place_object->GetWindow(&this->hwnd_control);
	return this->hwnd_control;
}
