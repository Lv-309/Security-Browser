...
namespace ISXFaceDetector
{
	...
	void FaceDetector::SaveWithInterval(int interval_in_sec /* = 5 */)
	{
		std::vector<cv::Rect> faces;
		...
		while (stop == false)
		{
			...
			if (interval_in_sec <= duration.count())
			{
				...
				if (faces.size() > 1)
				{
					...
					photo_name = std::to_wstring(num_of_many_faces) + L"th_photo_with_many_faces";
					//frame.SendToMoodle(photo_name);
					frame.SaveFrameToFile("C:\\Users\\theha\\Desktop\\Security-Browser-bad_version\\SecureBrowser v0.9.2\\Photos\\ManyFaces\\" + std::to_string(i) + ".jpg");
				}
				else if (faces.size() == 0)
				{
					num_of_no_face++;
					...
					//frame.SendToMoodle(photo_name);
					frame.SaveFrameToFile("C:\\Users\\theha\\Desktop\\Security-Browser-bad_version\\SecureBrowser v0.9.2\\Photos\\NoFace\\" + std::to_string(i) + ".jpg");
					//frame.SaveFrameToFile("..\\Photos\\NoFace\\" + std::to_string(i) + ".jpg");
				}
				else
				{
					num_of_one_face++;
					...
					//frame.SendToMoodle(photo_name);
					frame.SaveFrameToFile("C:\\Users\\theha\\Desktop\\Security-Browser-bad_version\\SecureBrowser v0.9.2\\Photos\\OneFace\\" + std::to_string(i) + ".jpg");
				}
